...
struct DxROM : BasicMapper {
	...
	DxROM(NesFile& nf, int variant) :BasicMapper(nf),
		prg_fixed(variant == VARIANT_DxROM_FixedPrg),
		or_mask(variant == VARIANT_DxROM_SplitChr ? 0x40 : 0x00) {}
};
DECLARE_MAPPER_INT(DxROM)
struct Namcot3453 : DxROM {
	void poweron() {
		DxROM::poweron();
		set_mirroring(MIRRORING_SCREENA);
	}
	void cpu_write(uint16_t addr, uint8_t data) {
		DxROM::cpu_write(addr, data);
		if (addr & 0x8000)
			set_mirroring(data & 0x40 ? MIRRORING_SCREENB : MIRRORING_SCREENA);
	}
	Namcot3453(NesFile& nf) :DxROM(nf, VARIANT_DxROM_SplitChr) {}
};
...
struct Namcot3425 : DxROM {
	...
	Namcot3425(NesFile& nf) :DxROM(nf, VARIANT_DxROM_Normal) {}
};
